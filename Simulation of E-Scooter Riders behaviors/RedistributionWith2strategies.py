...
RoutePriceList=[]
for j in range(len(PricingList)):
    RoutePriceList.append(PricingList[j]-OpsCostList[j])
...
#Simulation
#game theory based redistribution strategy
...
# test=open('dataAnalyticsWR3.csv','wb')
# tt=csv.writer(test)
    ...
    for t in range(m): #sum scoot should say fixed. it should not vary. balance transfer.
        ...
        for j in range(len(ScooterInitializeList)):
            if j==0: #MRT
                ...
                ExcesSupplyList.append(ScooterInitializeList[j]-ScooterFlow)
        ...
        if r==2: #restribution strategy 2: Game Theory concepts of stakelberg and cournet competition
            N=int(min(sum(LossDemandList),sum(ExcesSupplyList))) #decide no of people will evaluate his/her utility to redistribute
            NN+=N
            ExcessSupplyLocations=[]
            ExcessSupplyAtLocations=[]
            LossDemandLocations=[]
            LossDemandAtLocations=[]
            for j in range(len(ScooterInitializeList)): #seperate those with excess from those with loss demand
                if ExcesSupplyList[j]>LossDemandList[j]:
                    ExcessSupplyLocations.append(j)
                    ExcessSupplyAtLocations.append(ExcesSupplyList[j])
                elif ExcesSupplyList[j]<LossDemandList[j]:
                    LossDemandLocations.append(j)
                    LossDemandAtLocations.append(LossDemandList[j])

        # print "t",t,"N",N, "ExcessSupplyLocations",ExcessSupplyLocations,"ExcessSupplyAtLocations",ExcessSupplyAtLocations,"LossDemandLocations",LossDemandLocations,"LossDemandAtLocations",LossDemandAtLocations

        for j in range(len(ScooterInitializeList)): #balancing
            if j==0:
        ...
        if r==1: #restribution strategy 1: distribute 210 scooters at t=10 & 15 (Mrt to kiosk), distribute t= 58 & 63 (Kiosk to MRT)
            if t==10 or t==15:
        ...
        if r==2:    
            for k in range(N):
                ...
                if len(ExcessSupplyLocations)!=0 and len(LossDemandLocations)!=0:
                    DepartureLocation=random.choice(ExcessSupplyLocations)
                    ArrivalLocation=random.choice(LossDemandLocations)
                    
                    if ArrivalLocation==0:
                        RoutePrice=random.choice(RoutePriceList)
                        Destination=RoutePriceList.index(RoutePrice)+1
                        Cost=random.uniform(0,RoutePrice)
                        QuantityDelivered=RoutePrice/float(4*Cost+2)
                        PQ=RoutePrice/float(2)
                        ThresholdQuantity=((RoutePrice**2)+RoutePrice)/float(4*RoutePrice+2)
                        ThresholdUtility=PQ*ThresholdQuantity-Cost*(ThresholdQuantity**2)
                        IndividualUtility=PQ*QuantityDelivered-Cost*(QuantityDelivered**2)
                        UtilityDay+=IndividualUtility

                        if IndividualUtility>=ThresholdUtility:
                            ScooterInitializeList[DepartureLocation]-=1
                            ScooterInitializeList[Destination]+=1
                            # ScooterInitializeList[Destination]+=1 #no need, is taken into acc in the balancing stage
                            ExcessSupplyAtLocations[ExcessSupplyLocations.index(DepartureLocation)]-=1
                            LossDemandAtLocations[LossDemandLocations.index(ArrivalLocation)]-=1
                            LostDemand[0][ArrivalLocation]-=1 #loss demand decrease
                            Profit[0][Destination-1]+=RoutePrice-PQ #profit increase, utility of smrt
                            
                            if ExcessSupplyAtLocations[ExcessSupplyLocations.index(DepartureLocation)]==0:
                                ExcessSupplyLocations.pop(ExcessSupplyLocations.index(DepartureLocation))
                            elif LossDemandAtLocations[LossDemandLocations.index(ArrivalLocation)]==0:
                                LossDemandLocations.pop(LossDemandLocations.index(ArrivalLocation))
                            else:
                                pass
                        else:
                            pass
                    else: #other kiosks        
                        RoutePrice=RoutePriceList[ArrivalLocation-1]
                        Destination=0 #mrt
                        Cost=random.uniform(0,RoutePrice)
                        QuantityDelivered=RoutePrice/float(4*Cost+2)
                        PQ=RoutePrice/float(2)
                        ...
                        if IndividualUtility>=ThresholdUtility:
                            ...
                            LossDemandAtLocations[LossDemandLocations.index(ArrivalLocation)]-=1
                            LostDemand[0][ArrivalLocation]-=1 #loss demand decrease
                            Profit[0][ArrivalLocation-1]+=RoutePrice-PQ #profit increase, utility of smrt
...
if r==2:
    UtilityDay1=UtilityDay/float(NN)
    UtilityDay=UtilityDay/float(n)
    NNperday=NN/float(n)
# test.close()
...
if r==2:
    print "totalserviceproviderutility",UtilityDay,"AverageServiceProviderUtility",UtilityDay1, 'NNperday',NNperday
